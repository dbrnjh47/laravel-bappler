...
use App\Filament\Resources\ProxyResource\RelationManagers\BrowserProfilesRelationManager;
...
class ProxyResource extends Resource
{
    ...
    // protected static ?string $navigationLabel = 'Proxy';
        ...
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('uuid')
                    ->label('UUID')
                    ->searchable(),
                Tables\Columns\TextColumn::make('provider.login')
                    ->description(fn ($record): string => $record->provider->email)
                    ->label('Provider')
                    ->url(fn ($record): string => ProxyProviderResource::getUrl('view', ['record' => $record->proxy_provider_id]))
                    ->openUrlInNewTab()
                    ->color('primary')
    ...
    {
        return [
            BrowserProfilesRelationManager::class
        ];
    }
    ...
    public static function getPages(): array
    {
        return [
            ...
            'view' => Pages\ViewProxy::route('/{record}'),
